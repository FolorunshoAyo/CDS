<?php
    ...
    if(isset($_POST['submit'])){
        ...
        if(empty($pid) || empty($uid) || empty($total) || empty($amount)){
            ...
            // GET DEFAULT ADDRESS
            $sql_address = $db->query("SELECT * FROM user_addresses WHERE user_id={$user_id} AND active=1");
            ...
            $shipping_address = $default_address_details['recipient_name'] . "%" . $default_address_details['delivery_address'] . ", " . $default_address_details['city_name'] . ". " . $default_address_details['address_state'] . "." .  " (" . $default_address_details['recipient_postalcode'] . ") " . $default_address_details['recipient_phoneno'];
