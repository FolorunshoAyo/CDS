<?php
    ...
    if(isset($_POST['submit'])){
        ...
        if(empty($wid)){
            ...
            $sql_get_wallet_details = $db->query("SELECT agent_wallets.total_amount, products.name FROM agent_wallets INNER JOIN products ON agent_wallets.product_id = products.product_id WHERE wallet_id='$wid'");

            $wallet_details = $sql_get_wallet_details->fetch_assoc();

            echo json_encode(array('success' => 1, 'balance' => strval(intval($wallet_details['total_amount'])), 'name' => $wallet_details['name']));
        }
    }else{
