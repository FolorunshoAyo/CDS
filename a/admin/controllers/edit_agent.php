<?php
    ...
    if(isset($_POST['submit'])){
        ...
        if(empty($fname) || empty($lname) || empty($oname) || empty($phoneno) || empty($active)){
            echo json_encode(array('success' => 0, 'error_title' => "Create Agent", 'error_msg' => 'One or more fields are empty'));
        }else{
           // UODATE AGENT
            $sql_update_agent = $db->query("UPDATE agents SET first_name='$fname', last_name='$lname', other_name='$oname', account_status='$account_status'");
